    ...
    return (
        <div className="bg-background min-h-screen flex flex-col">
            ...
            <section className="relative h-[320px] flex items-center justify-center overflow-hidden bg-neutral-100">
                <Image src="/images/banner/banner2.jpg" alt="Techvalio Team Discussion" fill className="object-cover z-0" style={{ opacity: 0.74 }} />
                <div className="absolute inset-0 bg-black/40 z-0"></div>
                <motion.h1 className="relative z-10 text-white text-4xl sm:text-5xl font-bold text-center" initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
                    ABOUT US
                </motion.h1>
            </section>

            {/* COMPANY STORY - as one card */}
            <section className="container mx-auto px-4 -mt-16 mb-10 z-10">
                ...
                >
                    ...
                    <div className="p-7 md:p-10 flex-1">
                        <h2 className="font-headline text-2xl font-bold mb-2 text-primary">WE ARE TECHVALIO</h2>
                        <p className="text-muted-foreground mb-4">
                            Techvalio is a leader in software validation, digital compliance, and consulting for regulated businesses. <br />
                            We help pharma, healthcare, startups, and global enterprises launch audit-ready solutions with confidence.
                        </p>
                        <p className="text-sm text-muted-foreground mb-5">
                            Since our inception, we've empowered organizations to future-proof operations, reduce risk, and ensure every deployment exceeds both regulatory and market standards.
                        </p>
                        <div className="mt-3 italic text-primary font-semibold">"Modern Validation. Trusted Compliance."</div>
                        <div className="text-right font-bold mt-3 text-primary">~ Team Techvalio</div>
                    </div>
                    {/* Photo Right */}
                    <div className="flex-1 flex justify-center items-center  py-6 md:py-0 bg-white">
                        <Image src="/images/banner/banner3.jpg" alt="Suraj Randhari, CEO" width={320} height={480} className="object-cover w-full h-80 md:h-[380px] rounded-xl shadow" />
                    </div>
                </motion.div>
            </section>
            ...
            <section className="container mx-auto px-4 pb-12 md:py-14">
                <div className="grid md:grid-cols-2 gap-8 items-center">
                    <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
                        <Image src="/images/banner/banner5.jpg" alt="Techvalio Team At Work" width={430} height={270} className="rounded-xl shadow-xl" />
                    </motion.div>
                    <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
                        <h3 className="font-headline text-2xl font-bold mb-4">WHY CHOOSE US</h3>
                ...
                <div className="flex flex-wrap gap-8 justify-center">
                    {team.map(member => (
                        <motion.div
                            ...
                            <Image src={member.image} alt={member.name} width={96} height={96} className="rounded-full object-cover mb-2 shadow" />
                            <div className="font-semibold">{member.name}</div>
                            <div className="text-sm text-muted-foreground mb-2">{member.title}</div>
                            <div className="flex gap-3 text-primary text-lg">
                                {member.socials.map((s, i) => <a key={i} href={s.url} target="_blank" rel="noopener noreferrer">{s.icon}</a>)}
                            </div>
                        </motion.div>
                    ))}
                </div>
            ...
            {/* FOOTER (edit for your actual Info) */}
            <footer className="bg-muted py-10 mt-auto">
                <div className="container mx-auto text-xs text-muted-foreground flex flex-col md:flex-row justify-between gap-6">
                    <div>
                        ...
                        <div className="max-w-xs pt-1">Your partner for audit-ready, compliant software validation and digital transformation.</div>
                    </div>
                    <div>
